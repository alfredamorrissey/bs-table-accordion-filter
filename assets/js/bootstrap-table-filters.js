...
AccordionFilter.prototype.initAlertDiv = function() {
	...
	this.alertObj = new AccordionFilterAlerts($j('#'+options.msgdiv), this);
};
...
AccordionFilter.prototype.initFilterItem = function(panelGrp_div, key, item) {
	...
	$j(toggle_a).addClass("accordion-toggle").attr({"data-toggle":"collapse", "data-parent":"#accordion", "href":"#collapse_"+key});
	
	$j(panel_div).append(panelHd_div);
	$j(panelHd_div).append(toggle_a);
	$j(toggle_a).append("<h4 class=\"panel-title\">"+item.label+"<span class=\"fa fa-plus pull-right\" aria-hidden=\"true\"></span></h4>");
...
AccordionFilter.prototype.initSubfilter = function(panelBody_div, set_id, element) {
	...
	$j(toggle_a).addClass("accordion-toggle").attr({"data-toggle": "collapse", "data-parent":set_id, "href":"#collapse_"+set_id+element.value});
	$j(toggle_a).append("<span class=\"fa fa-plus\"></span>  "+element.label);
	$j(collapse_div).addClass("collapse offset1").attr({"id":"collapse_"+set_id+element.value});
...
AccordionFilter.prototype.handleOpenAccordion = function() {
    ...
    $j('#sidebarGroupAccordion .panel-heading').find('span').removeClass('fa fa-plus').addClass('fa fa-minus');
};

AccordionFilter.prototype.handleCloseAccordion = function() {
    $j('.panel-collapse.in').collapse('hide');  
    $j('#sidebarGroupAccordion .panel-heading').find('span').removeClass('fa fa-minus').addClass('fa fa-plus');
};
...
AccordionFilter.prototype.bindEvents = function() {
	...
	$j('.closeall').bind("click",filter_pointer.handleCloseAccordion);
    $j('.openall').bind("click",filter_pointer.handleOpenAccordion);
} ;
...
AccordionFilter.prototype.handleChangeFilterCheck = function(element, alertObj) {
	...
	cf_div = name+'_cf';
	message = $j(element).data("alert");//$j(element).parent().text();
...
$j.fn.accordionFilter = function(option) {
	this.each(function () {
		options = $j.extend({}, AccordionFilter.DEFAULTS, $j(this).data());
		$j(this).data = new AccordionFilter(this, options);
	});
};
